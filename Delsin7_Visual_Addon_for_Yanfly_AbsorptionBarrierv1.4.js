...
1.4- Fixed some potential actor access issues and erroneous requirements thanks to Azrael Astaroth. Added more menu UI options.
...
@plugindesc v1.04 Fixes some issues and adds more UI positional customization.
@author Delsin7

@param Display Barrier Gauge Seperately?
@type boolean
@desc Display Absorption gauge as a seperate gauge underneath the HP gauge? Default false.
@default false

@param Display Gauge Left to Right?
@type boolean
@desc Display Absorption gauge starting from left to right instead? Default false.
@default false
...
@option On
@option On Always
@desc Gauge will be randomized colors when it updates, if custom color notetags not used. On Always overrides notetags. Default Off.
@default Off
...
@option 1/3
@option 1/4
@desc Gauge will be shorter than the HP gauge. Default full.
@default full
...
@option center
@option top
@desc Aligns the gauge to the bottom, center, or top of HP Bar. Default bottom.
@default bottom

@param Display Barrier Numbers?
@type boolean
@desc Display Barrier number next to HP number gauge if 1 or higher? Default false.
@default false

@param Display Barrier Number Colors
@type number
@desc The color number that the Barrier Points will be on the HP gauge. Default: 0.
@default 0
...
@param Adjust Class Name X Position 
@type number
@desc The value to adjust the class name's x value in the main menu. Positive is to the right. Default: 0.
@default 0

@param Adjust Class Name Y Position 
@type number
@desc The value to adjust the class name's y value in the main menu. Positive is down. Default: 0.
@default 0

@param Adjust Bar Width 
@type number
@desc The value to adjust the hp, mp, bp, and tp bar width in the main menu. Positive is larger. Default: 0.
@default 0

@param Adjust Bar X Position 
@type number
@desc The value to adjust the hp, mp, bp, and tp bar x position in the main menu. Positive shifts to the right. Default: 0.
@default 0

@param Adjust Bar Y Position 
@type number
@desc The value to adjust the hp, mp, bp, and tp bar y position in the main menu. Positive shifts down. Default: 0.
@default 0
...
Delsin7.VAB.classNameAdjustX = Number(Delsin7.parameters["Adjust Class Name X Position"]);
Delsin7.VAB.classNameAdjustY = Number(Delsin7.parameters["Adjust Class Name Y Position"]);
Delsin7.VAB.adjustBarWidth = Number(Delsin7.parameters["Adjust Bar Width"]);
Delsin7.VAB.adjustBarX = Number(Delsin7.parameters["Adjust Bar X Position"]);
Delsin7.VAB.adjustBarY = Number(Delsin7.parameters["Adjust Bar Y Position"]);
...
Window_Base.prototype.barrierColor2 = function(actor) {
	...
	if(Delsin7.VAB.gaugeRandomColor.toLowerCase() == 'off'){
		newColor2 = Window_Base.prototype.barrierColor2Eval(actor);
	} else if (Delsin7.VAB.gaugeRandomColor.toLowerCase() != 'on always'){
		newColor2 = Window_Base.prototype.barrierColor2Eval(actor);
		if(newColor2 == -1){
...
Window_Base.prototype.barrierColor2Eval = function(actor) {
	...
	if(actor.isActor()){
		...
		newColorB = (actor.enemy().barrierColor2 !== undefined) ? actor.enemy().barrierColor2 : newColorB;
	}
...
Window_Base.prototype.drawActorSimpleStatus = function(actor, x, y, width) {
    ...
    var x2 = x + xpad + adjustBarX;
    var width2 = width - xpad - this.textPadding() + adjustBarWidth;//Math.max(280, width - xpad - this.textPadding());
    this.drawActorName(actor, x, y, 270);
	var omitLevel = $dataActors[actor.actorId()].meta.omitLevel
	var omitHp = $dataActors[actor.actorId()].meta.omitHp
	var omitBp = $dataActors[actor.actorId()].meta.omitBp
	var omitMp = $dataActors[actor.actorId()].meta.omitMp
	var omitTp = $dataActors[actor.actorId()].meta.omitTp
	//console.log(omitLevel);
	//console.log($dataActors[1]);
	//console.log($dataActors[1].name());
	//console.log($dataActors[1].meta);
	//console.log($dataActors[actor.actorId()].meta.Omit_Draw);
	//console.log(Delsin7.VAB.displayBarrierGauge);
	if(!omitLevel && !$dataActors[actor.actorId()].meta.Omit_Draw){
		this.drawActorLevel(actor, x, y + lineHeight * 1);
	};
    this.drawActorIcons(actor, x, y + lineHeight * 2);
    this.drawActorClass(actor, x2+Delsin7.VAB.classNameAdjustX, y+Delsin7.VAB.classNameAdjustY, width2);

	if(Delsin7.VAB.displayBarrierGauge == true && !$dataActors[actor.actorId()].meta.Omit_Draw) {//!$dataActors[actor._actorId[0]].meta.Omit_Draw){
		var mod = 1;

		if(!omitHp){
			this.drawActorHp(actor, x2, y + lineHeight * mod + adjustBarY, width2);
			mod += 1;
		};
		if(!omitBp){
			this.drawActorBp(actor, x2, y + lineHeight * mod + adjustBarY, width2);
			mod += 1;
		};
		if(!omitMp){
			this.drawActorMp(actor, x2, y + lineHeight * mod + adjustBarY, width2);
			mod += 1;
		};
		if(!omitTp){
			if (Yanfly.Param.MenuTpGauge) {
				this.drawActorTp(actor, x2, y + lineHeight * mod + adjustBarY, width2);
			}
		};
		
	} else {
		if(!$dataActors[actor.actorId()].meta.Omit_Draw) {//!$dataActors[actor._actorId[0]].meta.Omit_Draw){
			var mod = 1;
...
if(Imported.YEP_X_VisualHpGauge == true && Delsin7.VAB.displayBarrierGauge == true){
	...
	Window_VisualHPGauge.prototype.drawActorBp = function(actor, x, y, width) {
		width = width || 186;
		if(actor[Delsin7.VAB.maxBarrierParam] > 0){
			var max = actor[Delsin7.VAB.maxBarrierParam];
		} else {
			var max = actor.mhp;
		};
		var rate = actor.barrierPoints() / max;
		width = width || 186;
		if(Yanfly.Param.GaugeOutline == true){
			if(max != 0){
		...
		if (Yanfly.Param.VHGShowHP) {
			this.changeTextColor(this.systemColor());
			this.drawText(Delsin7.VAB.barrierParamAbbreviation, x, y-3, 45);
		}
		if (Yanfly.Param.VHGShowValue) {
		    var val = actor.barrierPoints();
		    var w = width;
		    var color = this.hpColor(actor);
		    this.drawCurrentAndMax(val, max, x, y-3, w, color, this.normalColor());
		}
